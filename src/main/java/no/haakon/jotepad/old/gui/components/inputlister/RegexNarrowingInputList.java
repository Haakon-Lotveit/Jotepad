...
/**
 * Dette var første forsøk på en inputliste som diskriminerer mens du skriver.
 * Den funker, men den er tung å sette opp.
 * SimpleNarrowingInputList er noe bedre, men ikke ferdig den heller...
 * @param <T> typen du vil ha listet ut.
 */
public abstract class RegexNarrowingInputList<T> {
    ...
    public RegexNarrowingInputList(String tittel, Collection<T> elementerTilListen, ApplicationFrame frame) {
     ...
     * som å ikke matche via regex. (I det tilfellet, er det lurt å overkjøre {@link RegexNarrowingInputList#mønsterFra(String)} også,
     * så en ikke lager patterns en ikke trenger. (En kan for eksempel kompilere ".*" og gjenbruke det objektet konstant.)
     * En får med både teksten som brukeren har skrevet ned, samt mønsteret som ble bygget tidligere i {@link RegexNarrowingInputList#oppdaterUtvalg(String)}.
     * @param tekst Teksten brukeren har skrevet i vinduet. Aldri null.
     * @param mønster Mønsteret som ble bygget tidligere. Med mindre {@link RegexNarrowingInputList#oppdaterUtvalg(String)} er overkjørt vil denne aldri være null heller.
     * @return en DefaultListModel som inneholder alle matchende elementer i {@link RegexNarrowingInputList#listen} som skal med videre.
